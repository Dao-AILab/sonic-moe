...
def moe_unsloth_layer(x, router_w, w1, w2, top_k):
    ...
    h = grouped_gemm(
        X=x, W=w1,
        m_sizes=m_sizes, gather_indices=gather_idx,
        topk=K, permute_x=True, permute_y=False,
        autotune=True, is_first_gemm=True,
    )                                                                  
    ...
    h = grouped_gemm(
        X=h, W=w2,
        m_sizes=m_sizes, gather_indices=gather_idx,
        topk=K, permute_x=False, permute_y=True,
        autotune=True, is_first_gemm=False,
    )                                                                  
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Unsloth MoE grouped_gemm benchmark (torch.compile + fastest paths)."
    )
...
def run(thiek, dtype):
    ...
    print0(f" Unsloth Fwd (training mode)         Average time: {fwd_train:.3f} ms, "
    ...
    print0(f"[bold green][/bold green] Unsloth Fwd + Bwd Average time: {e2e:.3f} ms, "
           f"TFLOPS: {e2e_flops / (e2e * 1e9):.1f}")

    bwd_flops = 12 * T * I * H * K
    bwd_time = e2e - fwd_train
    print0(f"[bold green][/bold green] Unsloth Bwd Average time: {bwd_time:.3f} ms, "
